...
function ReservationForm({ onSubmit }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    // Simulating a 3 second loading state
    setTimeout(() => {
      setLoading(false);
      onSubmit(formData);
    }, 4000);
    ...
    <div className="max-w-2xl mx-auto p-4 bg-white shadow-2xl rounded-xl border-2 border-red500">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <div>
            <label className="block mb-1 font-bold text-black">Start Date</label>
            <input 
              type="date"
              name="startDate"
          ...
          <div>
            <label className="block mb-1 font-bold text-black">End Date</label>
            <input 
              type="date"
              name="endDate"
